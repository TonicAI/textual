...
def test_simple_csv():
    ...
    with open(get_resource_path("single_convo.csv"), "r") as f:
        response = helper.redact(
            f, True, lambda x: x["id"], lambda x: x["text"], redact
        )

        assert len(response) == 2
        assert_redaction_response_equal(
            response[0], build_redaction_response_from_json(expected_row1)
        )
        assert_redaction_response_equal(
            response[1], build_redaction_response_from_json(expected_row2)
        )
...
def test_two_conversation_csv_simple():
    ...
    with open(get_resource_path("two_convo.csv"), "r") as f:
        ...
        assert_redaction_response_equal(
            response[2], build_redaction_response_from_json(expected_row3)
        )
        assert_redaction_response_equal(
            response[3], build_redaction_response_from_json(expected_row4)
        )
...
def test_single_border_crossing():
    ...
    def redact(x) -> RedactionResponse:
        return RedactionResponse(
            ...
            [
                Replacement(
                    16, 19, 16, 33, "NAME_GIVEN", "Lis", 0.9, "en", "[NAME_GIVEN_uWI2]"
                ),
                Replacement(
                    32,
                    37,
...
def test_null_grouping():
    ...
    def redact(x) -> RedactionResponse:
        return RedactionResponse(
            ...
            [
                Replacement(
                    3, 7, 3, 21, "NAME_GIVEN", "adam", 0.9, "en", "[NAME_GIVEN_ssYs5]"
                ),
                Replacement(
                    29,
                    35,
...
def test_invalid_row():
    ...
    def redact(x) -> RedactionResponse:
        return RedactionResponse("hello world", "hello world", 0, [])
...
def test_empty_csv():
    ...
    def redact(x) -> RedactionResponse:
        return RedactionResponse(x, x, 0, [])
    ...
    response = helper.redact(
        csv_file, True, lambda x: x["id"], lambda x: x["text"], redact
    )
...
def test_multiple_entity_overlaps():
    ...
    def redact(x) -> RedactionResponse:
        return RedactionResponse(
            ...
            [
                ...
                Replacement(
                    5, 10, 5, 19, "PERSON", "Smith", 0.9, "en", "[PERSON_DfhGqE]"
                ),
                Replacement(
                    16,
                    29,
    ...
    assert (
        response[0].original_text
        == "John Smith from New York City works at Tonic AI in Seattle"
    )
    assert (
        response[0].redacted_text
        == "John [PERSON_DfhGqE] from [LOCATION_CITY_KAjfG3] works at [ORGANIZATION_5Ve7OH] in [LOCATION_CITY_Jaf4DF]"
    )
